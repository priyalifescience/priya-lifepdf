...
<div align="center">
  <img src="public/images/logo.png" alt="Priya LifePDF Logo" width="120" height="120" />
...
[![Website](https://img.shields.io/website?url=https%3A%2F%2Fpriya-lifepdf.devtoolcafe.com%2Fen%2F)](https://priya-lifepdf.devtoolcafe.com/en/)
...
**Priya LifePDF** is a comprehensive suite of PDF tools designed for privacy and performance. Unlike many online converters, Priya LifePDF processes your files entirely within your browser using WebAssembly technology. Your documents **never** leave your device, ensuring maximum security for your sensitive data.
...
Priya LifePDF includes a powerful **visual workflow editor** that allows you to chain multiple PDF operations together, creating automated processing pipelines.
...
1.  **Clone the repository**
    ```bash
    git clone https://github.com/Priya LifePDFTool/priya-lifepdf.git
    cd priya-lifepdf
    ```
...
Priya LifePDF provides both pre-built Docker images and Docker Compose for flexible deployment options.

#### Option 1: Use Pre-built Image (Recommended)

The easiest way to run Priya LifePDF is using our pre-built image from GitHub Container Registry:

```bash
# Pull the latest image
docker pull ghcr.io/priya-lifepdftool/priya-lifepdf:latest

# Run the container
docker run -d -p 8080:80 --name priya-lifepdf ghcr.io/priya-lifepdftool/priya-lifepdf:latest
```

Open [http://localhost:8080](http://localhost:8080) to access Priya LifePDF.
...
Priya LifePDF supports deployment under a subpath (e.g., `https://your-domain.com/priya-lifepdf/`). To enable this, you must specify the base path during the build process.

**Using Docker Build:**
```bash
docker build --build-arg BASE_PATH=/priya-lifepdf -t priya-lifepdf .
```
...
Priya LifePDF is configured for static export (`output: 'export'`), which means it can be deployed to any service that supports static website hosting without requiring a Node.js server.
...
Priya LifePDF stands on the shoulders of giants. We gratefully acknowledge [BentoPDF](https://github.com/alam00000/bentopdf) for their pioneering work in privacy-first, client-side PDF tools.

Their project served as a significant inspiration and reference for our core logic. While Priya LifePDF has been re-engineered for the Next.js ecosystem and extends functionality with unique features like the *Workflow Editor*, we deeply respect the foundation laid by the BentoPDF team.
...
<div align="center">
  Built with ❤️ by the Priya LifePDF Team
</div>
