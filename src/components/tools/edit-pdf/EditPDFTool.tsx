    ...
    setTimeout(() => {
      ...
      try {
        ...
        if (iframe?.contentDocument) {
          ...
          }          // 3. Inject Priya LifePDF Enrichment Script
          const patchScript = doc.createElement('script');
          patchScript.textContent = `
            (function() {
              console.log('[Priya LifePDF Patch] Initializing annotation patches...');
              ...
              const initInterval = setInterval(() => {
                ...
                if (ext) {
                  ...
                  console.log('[Priya LifePDF Patch] pdfjsAnnotationExtensionInstance found! Setting up patches...');
                  setupCloudFix();
              ...
              function setupSnapping() {
                ...
                console.log('[Priya LifePDF Patch] Setting up Konva Snapping Alignment...');
                ...
                function drawGuides(stg, sx, sy) {
                  let container = document.getElementById('priya-lifepdf-alignment-guides');
                  if (!container) {
                    container = document.createElement('div');
                    container.id = 'priya-lifepdf-alignment-guides';
                ...
                function clearGuides() {
                  const container = document.getElementById('priya-lifepdf-alignment-guides');
              ...
              function setupChineseFontPatch() {
                ...
                pdfLib.PDFDocument.prototype.save = async function(saveOptions) {
                  console.log('[Priya LifePDF Patch] Intercepting save to inspect for Chinese text...');
                  ...
                  if (hasChinese) {
                    try {
                      console.log('[Priya LifePDF Patch] Chinese text found. Embedding NotoSansSC-Regular font...');
                      ...
                      this.embedFont = async function(fontToEmbed, embedOpts) {
                        if (fontToEmbed === pdfLib.StandardFonts.Helvetica || fontToEmbed === 'Helvetica') {
                          console.log('[Priya LifePDF Patch] Redirected Helvetica embed to NotoSansSC font');
                          return customFont;
                        }
                        return originalEmbedFont.call(this, fontToEmbed, embedOpts);
                      };
                    } catch (e) {
                      console.error('[Priya LifePDF Patch] Failed to embed Chinese font subset', e);
                    }
                  }
              ...
              function setupCloudFix() {
                ...
                document.addEventListener('dblclick', function(e) {
                  ...
                  if (activeTool === 'cloud') {
                    ...
                    if (konvaContent) {
                      console.log('[Priya LifePDF Patch] Intercepted dblclick for cloud tool, dispatching to Konva stage.');
                ...
                document.addEventListener('keydown', function(e) {
                  if (e.key === 'Enter') {
                    ...
                    if (activeTool === 'cloud') {
                      const konvaContent = document.querySelector('.konvajs-content');
                      if (konvaContent) {
                        console.log('[Priya LifePDF Patch] Intercepted Enter key for cloud tool, dispatching dblclick to end drawing.');
                        const dblEvent = new MouseEvent('dblclick', {
                          bubbles: true,
                          cancelable: true,
                          view: window
              ...
              function setupColorPickerAndStroke() {
                ...
                if (hlColorPicker) {
                  if (!hlColorPicker.querySelector('.priya-lifepdf-custom-hl-picker')) {
                    const picker = document.createElement('input');
                    picker.type = 'color';
                    picker.className = 'priya-lifepdf-custom-hl-picker';
              ...
              function injectCustomMenuControls(menu) {
                if (menu.querySelector('.priya-lifepdf-custom-controls')) return;

                console.log('[Priya LifePDF Patch] CustomAnnotationMenu opened, injecting custom controls...');

                const container = document.createElement('div');
                container.className = 'priya-lifepdf-custom-controls';
                ...
                nativeSliders.forEach(slider => {
                  if (slider.getAttribute('min') === '1') {
                    ...
                    console.log('[Priya LifePDF Patch] Stroke width slider updated min to 0');
                  }
                });
              ...
              function loadState(stateStr) {
                ...
                try {
                  ...
                  console.error('[Priya LifePDF Patch] Failed to load state', err);
                } finally {
                  setTimeout(() => {
              ...
              function injectUndoRedoButtons() {
                ...
                if (customToolbar) {
                  if (customToolbar.querySelector('.priya-lifepdf-undo-btn')) return;
                  const btnList = customToolbar.querySelector('ul') || customToolbar;

                  const undoLi = document.createElement('li');
                  undoLi.className = 'priya-lifepdf-undo-btn';
                  ...
                  redoLi.className = 'priya-lifepdf-redo-btn';
              ...
              function updateUndoRedoButtonsState() {
                const undoBtn = document.querySelector('.priya-lifepdf-undo-btn button');
                const redoBtn = document.querySelector('.priya-lifepdf-redo-btn button');
          ...
          console.log('[Priya LifePDF Patch] Enrichment script successfully injected into iframe!');
        }
      } catch (e) {
