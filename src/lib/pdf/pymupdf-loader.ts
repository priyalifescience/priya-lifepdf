...
max_dpi = ${customDpi || settings.maxDpi}
remove_metadata = ${removeMetadata ? 'True' : 'False'}
compression_quality = "${quality}"
force_grayscale = ${forceGrayscale ? 'True' : 'False'}
discard_annotations = ${discardAnnotations ? 'True' : 'False'}
discard_forms = ${discardForms ? 'True' : 'False'}
...
for page_num in range(len(doc)):
    ...
    for img_info in image_list:
        ...
        try:
            ...
            # Create pixmap from xref directly
            ...
            if pix.width > 100 and pix.height > 100:
                ...
                if pix.width > max_dpi * 10 or pix.height > max_dpi * 10:
                    ...
                    if scale < 1.0:
                        ...
                        if new_width > 30 and new_height > 30:
                            # Create new smaller pixmap by sampling
                            pix2 = pymupdf.Pixmap(pix, width=new_width, height=new_height)
                            pix = pix2
                
                # Convert to RGB or Gray (JPEG doesn't support alpha/CMYK well)
                is_gray = (pix.colorspace == pymupdf.csGRAY)
                if compression_quality == 'low' or force_grayscale:
                    pix = pymupdf.Pixmap(pymupdf.csGRAY, pix)
                    is_gray = True
                elif pix.alpha or (pix.colorspace != pymupdf.csRGB and pix.colorspace != pymupdf.csGRAY):
                ...
                if len(new_image_bytes) < len(image_bytes) * 0.9:
                    ...
                    # Remove DecodeParms, SMask, and Decode that may be left from PNG/Flate encoding
                    try:
...
# Discard annotations if requested
...
# Subset fonts to strip unused glyphs
try:
    doc.subset_fonts()
except:
    pass
