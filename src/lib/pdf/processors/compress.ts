...
import { compressWithPureEngine } from './compress-engine';
...
export interface CompressPDFOptions {
  ...
  /** Convert colorspace to grayscale to save space */
  forceGrayscale?: boolean;
  /** Remove annotations from the PDF */
  discardAnnotations?: boolean;
  /** Remove interactive form fields from the PDF */
  discardForms?: boolean;
  /** Custom resolution override in DPI */
  customDpi?: number;
  /** Custom JPEG quality override for standard/photon engines (0.01 to 1.0) */
...
export class CompressPDFProcessor extends BasePDFProcessor {
  ...
  async process(
    ...
    try {
      ...
      switch (compressOptions.algorithm) {
        ...
        case 'standard':
        default:
          // Pure-JS engine: structural pdf-lib pass + canvas image recompression
          result = await compressWithPureEngine(
            arrayBuffer,
            compressOptions,
            (pct, label) => this.updateProgress(pct, label)
          );
          break;
      }
  ...
  private async compressWithCondense(
    ...
    const result: Blob = await pymupdf.compress(file, {
      ...
      discardAnnotations: options.discardAnnotations,
      discardForms: options.discardForms,
      customDpi: options.customDpi,
    });
  ...
  private async compressWithPhoton(
    ...
    const dpi = options.customDpi || options.photonDpi || 150;
    const format = options.photonFormat || 'jpeg';
    const quality = options.customJpegQuality ? Math.round(options.customJpegQuality * 100) : (options.photonQuality || 85);
